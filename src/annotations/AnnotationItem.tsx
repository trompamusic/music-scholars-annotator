...
> {
  ...
  render() {
    ...
    let permission;
    let modifyPermissionsElement;
    let motivation = this.props.annotation.annotation.motivation;
    let bodyAnno;
    const body = this.props.annotation.annotation.body[0]!.value!;
    const bodyLinking = this.props.annotation.annotation.body[0]!.id!;
    ...
    switch (motivation) {
      case "describing":
        ...
        break;
      case "linking":
        if (bodyLinking.startsWith("https")) {
          bodyAnno = (
            <a href={bodyLinking} target="_blank" rel="noopener noreferrer">
              {bodyLinking}
            </a>
          );
        } else {
          const appendURL = "https://" + bodyLinking;
          bodyAnno = (
            <a href={appendURL} target="_blank" rel="noopener noreferrer">
              {appendURL}
            </a>
          );
        }
      ...
      case "trompa:cueImage":
        ...
        break;
      case "trompa:cueMedia":
        bodyAnno = bodyLinking + " this is not yet implemented";
        break;
      default:
        console.log("no definition for this body");
    }
      ...
      <>
        <Card className="ml-3">
          ...
          <Collapse in={this.state.showDetails}>
            <Card.Footer>
              <small>
                Created on:{this.props.annotation.annotation.created}
                <br />
                Permissions: {modifyPermissionsElement}
              </small>
            </Card.Footer>
        ...
        {this.state.showReplies &&
          this.props.annotation.replies.map((reply) => {
            return (
              <Card style={{ marginLeft: "1em" }} key={reply["@id"]}>
                <Card.Body>
                  <Card.Text>Reply: {reply.body[0]!.value!}</Card.Text>
                </Card.Body>
              </Card>
            );
          })}
