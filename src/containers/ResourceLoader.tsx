...
import MultiModalComponent, {SearchConfig, SearchResult} from 'trompa-multimodal-component';
...
const renderSearchResult = (type: string, item: any, onClick: (item: any) => void) => {
        ...
        <Col>
            ...
            <ul>
                {sampleScores.map(score => {
                    return <li key={score.url}>
                        <a href={`/annotate?resource=${score.url}`}
                           onClick={(e) => {
                               e.preventDefault();
                               loadUrl(score.url);
                           }}>{score.name}</a></li>
            ...
            <Form onSubmit={(e) => {
                ...
                <Row>
                    <Col sm={11}>
                        <Form.Control type="input" placeholder="Enter URL"
                                      value={userUrl}
                                      onChange={(e) =>
                                          setUserUrl(e.target.value)
                                      } />
                    </Col>
                    <Col><Button variant="primary" type="submit" onClick={() => {
                        if (userUrl && userUrl !== '') {
                            loadUrl(userUrl);
                        }
                    }}>Load</Button></Col>
