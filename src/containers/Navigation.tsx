...
import {handleIncomingRedirect, ISessionInfo, onSessionRestore} from "@inrupt/solid-client-authn-browser";
...
export default function Navigation() {
    ...
    return (
        <Navbar bg="light" expand="lg">
            <Navbar.Brand href="#home"><img src={Logo} alt="Trompa logo" />Music scholars annotation tool</Navbar.Brand>
            ...
            <Nav>
                { session && session.isLoggedIn
                  ? <Navbar.Text>
                    <CombinedDataProvider datasetUrl={session.webId!} thingUrl={session.webId!}>
                      Logged in: <Text property={FOAF.name.iri.value}/>
                      </CombinedDataProvider>
                    &emsp;
                    <LogoutButton onLogout={() => setSession(undefined)}><Button>Log out</Button></LogoutButton>
                    </Navbar.Text>
                  : <LoginButton oidcIssuer={trompaIdp} redirectUrl={window.location.origin}>
